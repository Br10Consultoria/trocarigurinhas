...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#f7faff] text-slate-950">
      <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/90 backdrop-blur"><div className="container flex h-16 items-center justify-between gap-5"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-700 text-white"><Sparkles className="h-4 w-4" /></div><div><p className="text-sm font-bold leading-none text-blue-950">Troca<span className="text-orange-500">Figurinhas</span></p><p className="mt-1 hidden text-[10px] font-medium uppercase tracking-[0.16em] text-slate-400 sm:block">Marketplace de colecionadores</p></div></div><div className="flex items-center gap-2"><NotificationCenter /><div className="hidden items-center gap-2 rounded-xl bg-blue-50 px-3 py-2 text-xs text-blue-800 sm:flex"><UserRound className="h-4 w-4" />{user?.name ?? "Minha conta"}</div><Button variant="ghost" size="sm" className="rounded-xl text-slate-500 hover:bg-slate-100" onClick={() => void logout()}><LogOut className="mr-2 h-4 w-4" />Sair</Button></div></div></header>
